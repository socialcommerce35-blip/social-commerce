...
export const authenticate = (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  try {
    const decoded = jwt.verify(token, config.jwtSecret) as { user_id: string; mobile: string, role:string };
    req.user = { user_id: decoded.user_id, mobile: decoded.mobile, role: decoded.role };
    next();
  } catch (err) {
